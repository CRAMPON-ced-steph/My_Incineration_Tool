...
const COOLINGTOWERFlueGasParameters = ({ innerData, currentLanguage = 'fr' }) => {
  ...
  const [emissions_COOLINGTOWER, setEmissions_COOLINGTOWER] = useState(() => {
    const savedEmissions = localStorage.getItem('emissions_COOLINGTOWER');
    return savedEmissions ? JSON.parse(savedEmissions) : initialEmissions_COOLINGTOWER;
  });

  useEffect(() => {
    localStorage.setItem('emissions_COOLINGTOWER', JSON.stringify(emissions_COOLINGTOWER));
  }, [emissions_COOLINGTOWER]);
  ...
  const T_out = emissions_COOLINGTOWER['Flue gas temperature outlet [°C]'];
  const T_air = emissions_COOLINGTOWER['Ambient air temperature [°C]'];
  const V_air_ingress = emissions_COOLINGTOWER['Volume of air ingress [Nm3/h]'];
  const Pth = emissions_COOLINGTOWER['Thermal losses [%]'];
  const T_eau = emissions_COOLINGTOWER['Cooling water temperature [°C]'];
  ...
  let Q_eau_kg_h = 0;
  let Delta_H = 0;
  let H_in_COOLINGTOWER = 0;
  let H_out_COOLINGTOWER = 0;
  ...
  if (V_air_ingress !== 0) {
    ...
    H_in_COOLINGTOWER = h_fumee(T_IN, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    H_out_COOLINGTOWER = h_fumee(T_out + (T_out - T_with_air_ingress_out), FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    Delta_H = H_in_COOLINGTOWER * (1 - Pth / 100) - H_out_COOLINGTOWER;
    ...
    H_in_COOLINGTOWER = h_fumee(T_IN, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    H_out_COOLINGTOWER = h_fumee(T_out, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    Delta_H = H_in_COOLINGTOWER * (1 - Pth / 100) - H_out_COOLINGTOWER;
    Q_eau_kg_h = Qeau_added_to_be_at_T(T_IN, T_eau, T_out, Pth, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
  }

  // Output composition
  const masses_FG_in_COOLINGTOWER = {
    CO2: FG_CO2_kg_h,
    O2: FG_O2_kg_h,
    H2O: FG_H2O_kg_h,
    N2: FG_N2_kg_h
  ...
  const masses_FG_out_COOLINGTOWER = {
  ...
  const FG_CO2_EAU_m3_h = CO2_kg_m3(masses_FG_out_COOLINGTOWER.CO2);
  const FG_H2O_EAU_m3_h = H2O_kg_m3(masses_FG_out_COOLINGTOWER.H2O);
  const FG_O2_EAU_m3_h = O2_kg_m3(masses_FG_out_COOLINGTOWER.O2);
  const FG_N2_EAU_m3_h = N2_kg_m3(masses_FG_out_COOLINGTOWER.N2);
  ...
  const elementsGeneric = [
    { text: t('Temperature inlet COOLINGTOWER [°C]'), value: T_IN.toFixed(1) },
  ...
  const handleChange = (name, value) => {
    ...
    setEmissions_COOLINGTOWER((prev) => ({
      ...prev,
      [name]: value,
    }));
  };
  ...
  const clearMemory = useCallback(() => {
    localStorage.removeItem('emissions_COOLINGTOWER');
    setEmissions_COOLINGTOWER(initialEmissions_COOLINGTOWER);
  }, []);
    ...
    <div className="cadre_pour_onglet">
      ...
      <div className="cadre_param_bilan">
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          {Object.entries(emissions_COOLINGTOWER).map(([key, value]) => (
            <div
              key={key}
              style={{
                display: 'flex',
      ...
      <MassCalculator masses={masses_FG_in_COOLINGTOWER} TemperatureImposee={T_IN} />
      ...
      <MassCalculator masses={masses_FG_out_COOLINGTOWER} TemperatureImposee={T_out} />
    </div>
  );
};

export default COOLINGTOWERFlueGasParameters;
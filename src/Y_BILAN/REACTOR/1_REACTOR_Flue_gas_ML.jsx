...
const REACTORFlueGasParameters = ({ innerData, currentLanguage = 'fr' }) => {
  ...
  const T_out = emissions_REACTOR['Flue gas temperature outlet [°C]'];
  const T_air = emissions_REACTOR['Ambient air temperature [°C]'];
  const V_air_ingress = emissions_REACTOR['Volume of air ingress [Nm3/h]'];
  const Pth = emissions_REACTOR['Thermal losses [%]'];
  const T_eau = emissions_REACTOR['Cooling water temperature [°C]'];
  ...
  if (V_air_ingress !== 0) {
    ...
    H_in_REACTOR = h_fumee(T_IN, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    H_out_REACTOR = h_fumee(T_out + (T_out - T_with_air_ingress_out), FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    Delta_H = H_in_REACTOR * (1 - Pth / 100) - H_out_REACTOR;
    ...
    H_in_REACTOR = h_fumee(T_IN, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    H_out_REACTOR = h_fumee(T_out, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
    Delta_H = H_in_REACTOR * (1 - Pth / 100) - H_out_REACTOR;
    Q_eau_kg_h = Qeau_added_to_be_at_T(T_IN, T_eau, T_out, Pth, FG_IN.CO2, FG_IN.H2O, FG_IN.N2, FG_IN.O2);
  }

  // Output composition
  const masses_FG_in_REACTOR = {
    CO2: FG_CO2_kg_h,
    O2: FG_O2_kg_h,
    H2O: FG_H2O_kg_h,
    N2: FG_N2_kg_h
  ...
  const FG_CO2_EAU_m3_h = CO2_kg_m3(masses_FG_out_REACTOR.CO2);
  const FG_H2O_EAU_m3_h = H2O_kg_m3(masses_FG_out_REACTOR.H2O);
  const FG_O2_EAU_m3_h = O2_kg_m3(masses_FG_out_REACTOR.O2);
  const FG_N2_EAU_m3_h = N2_kg_m3(masses_FG_out_REACTOR.N2);
  ...
  if (innerData) {
    ...
    innerData.FG_sec_EAU_tot = FG_sec_EAU_tot_m3_h;
    ...
    <div className="cadre_pour_onglet">
      ...
      <div className="cadre_param_bilan">
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          {Object.entries(emissions_REACTOR).map(([key, value]) => (
            <div
              key={key}
              style={{
                display: 'flex',
      ...
      <MassCalculator masses={masses_FG_in_REACTOR} TemperatureImposee={T_IN} />
      ...
      <MassCalculator masses={masses_FG_out_REACTOR} TemperatureImposee={T_out} />
    </div>
  );
};

export default REACTORFlueGasParameters;
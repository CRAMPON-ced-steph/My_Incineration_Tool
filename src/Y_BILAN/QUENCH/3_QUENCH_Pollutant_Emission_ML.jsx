...
const QUENCHFlueGasPollutantEmission = ({ innerData, currentLanguage = 'fr', nodeId }) => {
  ...
  const [emissionsQUENCH, setEmissions2] = useState(() => {
    const savedEmissions = localStorage.getItem(`emissionsQUENCH_${nodeId}`);
  ...
  useEffect(() => {
    localStorage.setItem(`emissionsQUENCH_${nodeId}`, JSON.stringify(emissionsQUENCH));
  }, [emissionsQUENCH]);

  // Extract parameters from state
  const FlyAsh_g_Nm3 = emissionsQUENCH['Fly ashes content [g/Nm3]'];
  const Bottom_Ash_Siccity = emissionsQUENCH['siccity bottom ash [%]'];
  const O2ref = emissionsQUENCH['O2 ref [%]'];

  const SOx_reactif = emissionsQUENCH['SOx reactif'];
  const HCl_reactif = emissionsQUENCH['HCl reactif'];
  const HF_reactif = emissionsQUENCH['HF reactif'];

  const efficacite_SOx = emissionsQUENCH['efficacite_SOx'];
  const HCl_efficacité = emissionsQUENCH['HCl efficacité'];
  const HF_efficacité = emissionsQUENCH['HF efficacité'];

  const SOx_stoechiométrie = emissionsQUENCH['SOx stoechiométrie'];
  const HCl_stoechiométrie = emissionsQUENCH['HCl stoechiométrie'];
  const HF_stoechiométrie = emissionsQUENCH['HF stoechiométrie'];
  ...
  const clearMemory = useCallback(() => {
    localStorage.removeItem(`emissionsQUENCH_${nodeId}`);
    setEmissions2(initialEmissions_QUENCH);
  }, []);
...
export default QUENCHFlueGasPollutantEmission;
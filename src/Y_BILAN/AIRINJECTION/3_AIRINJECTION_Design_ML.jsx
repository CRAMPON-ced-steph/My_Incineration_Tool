...
const AIRINJECTIONDesign = ({ innerData, setInnerData, currentLanguage = 'fr' }) => {
  ...
  // Default parameters
  const defaultParameters_airinjection = {
  ...
  // State management
  const [Parameters_airinjection, setParameters_airinjection] = useState(() => {
    return {
      FlueGasFlow_Nm3_h: innerData?.FG_humide_tot || defaultParameters_airinjection.FlueGasFlow_Nm3_h,
      Tempe_flue_gas_C: defaultParameters_airinjection.Tempe_flue_gas_C,
      VelocityTarget: defaultParameters_airinjection.VelocityTarget,
    };
  });
  ...
  // Extract parameters
  const FlueGasFlow_Nm3_h = Parameters_airinjection['FlueGasFlow_Nm3_h'];
  const Tempe_flue_gas_C = Parameters_airinjection['Tempe_flue_gas_C'];
  const VelocityTarget = Parameters_airinjection['VelocityTarget'];
  ...
  useEffect(() => {
    ...
    const flueGasFlow = Parameters_airinjection.FlueGasFlow_Nm3_h;
    const temperature = Parameters_airinjection.Tempe_flue_gas_C;
    const velocityTarget = Parameters_airinjection.VelocityTarget;
  ...
  }, [selectedModel, Parameters_airinjection]);
  ...
  const handleChange = (name, value) => {
    ...
    if (!isNaN(numericValue)) {
      if (name === 'VelocityTarget') {
        ...
        setParameters_airinjection(prev => ({ ...prev, [name]: limitedValue }));
      } else {
        setParameters_airinjection(prev => ({ ...prev, [name]: numericValue }));
      }
    }
  };
...
export default AIRINJECTIONDesign;

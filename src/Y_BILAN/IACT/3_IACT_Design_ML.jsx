...
const IACTDesign = ({ innerData, setInnerData, currentLanguage = 'fr' }) => {
  ...
  const [evacuation_IACT_ash, setEvacuation_IACT_ash] = useState({
  ...
  const type_camion = evacuation_IACT_ash['Type de camion'];
  const distance_km = evacuation_IACT_ash['Distance [km]'];
  ...
  const handleParametresChange = (name, value) => {
    ...
    } else if (name in evacuation_IACT_ash) {
      setEvacuation_IACT_ash(prev => ({ ...prev, [name]: typeof value === 'string' ? value : numericValue }));
    }
  };
    ...
    <div className="cadre_pour_onglet">
      ...
      >
        <div style={{ display: 'flex', gap: '20px', marginBottom: '20px' }}>
          <div style={{ flex: 1 }}>
            <img src={BHFimage} alt="IACT" style={{ width: '100%', maxWidth: '300px', objectFit: 'contain' }} />
          </div>

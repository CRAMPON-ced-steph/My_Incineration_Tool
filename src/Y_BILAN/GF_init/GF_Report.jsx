/**
 * GF_Report.jsx
 * Rapport de synthèse bilan du four à grille (Grate Furnace) — mode Bilan.
 *
 * Structure calquée sur AIRINJECTION_Report.jsx.
 * Les données sont lues directement depuis innerData (fourni par GFMainPage).
 */
...
const PollutantTable = ({ masses = {} }) => {
  ...
  return (
    <table style={styles.table}>
      <thead><tr>{keys.map(k => <th key={k} style={styles.th}>{k}</th>)}</tr></thead>
      <tbody><tr>{keys.map(k => <td key={k} style={styles.td}>{fmt(masses[k], 4)}</td>)}</tr></tbody>
    </table>
  );
};
...
const ElecTable = ({ rows, t }) => (
  <table style={styles.table}>
    ...
    <tbody>
      ...
      <tr style={{ background: '#eaf0fb', fontWeight: 'bold' }}>
...
const computeOpexCosts = (innerData) => {
  ...
  const eauRows = [
    ...
    { label: 'Eau refroidissement',  m3h: d.Conso_EauRefroidissement_m3|| 0, prix: waterPrices?.cooling       || 0 },
  ...
  const reactifRows = [
    ...
    { label: 'Ca(OH)₂',kgh: d.Conso_CaOH2_dry_kg || 0, prix: reagentsTypes?.CaOH2?.cost    || 0, co2T: reagentsTypes?.CaOH2?.co2PerTrip    || 0 },
  ...
  return {
    ...
    totalCout_h, totalCout_an, totalCO2_kgh, currency, availability,
  };
};

const OpexSummary = ({ opex, t }) => {
  const {
    totalElec_kW, coutElec, co2Elec, coutAir, co2Air, coutEau,
    coutReactifs, co2TransportReactifs, coutEnergie, co2Energie,
    coutTransportResidus, co2TransportResidus, coutTransportReactifs,
    totalCout_h, totalCout_an, totalCO2_kgh, currency, availability,
  } = opex;

  if (totalElec_kW === 0 && coutEnergie === 0 && coutEau === 0) {
    return <p style={{ color: '#999', fontSize: 12, padding: '10px 14px' }}>{t('Coûts OPEX non disponibles — ouvrir les onglets Design et Opex.')}</p>;
  }
  return (
    <div>
      <div style={styles.subSection}>
        <div style={styles.tagRow}>
          {[
            ...
            { label: `${t('Eau')} [${currency}/h]`,           val: coutEau,      color: '#2ecc71' },
            { label: `${t('Réactifs')} [${currency}/h]`,      val: coutReactifs, color: '#e74c3c' },
            { label: `${t('Énergie')} [${currency}/h]`,       val: coutEnergie,  color: '#f39c12' },
          ].map(({ label, val, color }) => (
            <div key={label} style={{ ...styles.tag, borderLeft: `4px solid ${color}`, minWidth: 130 }}>
...
// ── Composant principal ───────────────────────────────────────────────────────
const GF_Report = ({ innerData = {}, currentLanguage = 'fr' }) => {
  const languageCode = getLanguageCode(currentLanguage);
  const t = (key) => translations[languageCode]?.[key] || translations['fr']?.[key] || key;

  // Données fumées (amont depuis innerData)
  const FG_OUT_kg_h  = innerData.FG_OUT_kg_h  || {};
  const FG_OUT_Nm3_h = innerData.FG_OUT_Nm3_h || (() => {
    const c = {
    ...
    c.dry = c.CO2 + c.O2 + c.N2;
    c.wet = c.dry + c.H2O;
    return c;
  })();

  const T_OUT         = innerData.T_OUT         || 0;
  const O2_calcule    = (innerData.O2_calcule   || 0) * 100;
  const FG_wet_total  = (FG_OUT_kg_h.CO2 || 0) + (FG_OUT_kg_h.H2O || 0)
                      + (FG_OUT_kg_h.O2  || 0) + (FG_OUT_kg_h.N2  || 0);

  // Bilan énergétique
  const GF_result      = innerData.GF_result || {};
  const INCI           = GF_result.INCI || {};
  const P_inci_kW      = GF_result.P_incinerateur_kWH || 0;
  const PCI_kCal_kg    = GF_result.PCI_kCal_kg || 0;
  const PCI_kJ_kg      = PCI_kCal_kg * 4.1868;
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.mainTitle}>{t('GF — Rapport de synthèse')}</h1>

      {/* 1. Bilan énergétique */}
      <Section title={t('1. Bilan énergétique')}>
        <div style={styles.twoCol}>
          <SubSection title="Puissances">
            <KV label="Puissance incinérateur" value={fmt(P_inci_kW, 0)} unit="kW" />
            <KV label="Puissance incinérateur" value={fmt(P_inci_kW / 1000, 3)} unit="MW" />
            <KV label="PCI déchet"             value={fmt(PCI_kCal_kg, 0)} unit="kcal/kg" />
            <KV label="PCI déchet"             value={fmt(PCI_kJ_kg, 0)}   unit="kJ/kg" />
          </SubSection>
          <SubSection title="Récupération">
        ...
        <div style={styles.twoCol}>
          <SubSection title="Vapeur">
            ...
            <KV label="H eau surchauffée"   value={fmt(INCI.H_superheated_water_kW, 0)}  unit="kW" />
            <KV label="H pertes thermiques" value={fmt(INCI.H_pertes_thermiques_kW, 3)}  unit="kW" />
            <KV label="H imbrûlés"          value={fmt(INCI.H_imbrule_kW, 3)}            unit="kW" />
          </SubSection>
          <SubSection title="Entrées thermiques">
            <KV label="H air combustion"   value={fmt(INCI.H_air_comb_kW, 0)}             unit="kW" />
            <KV label="H eau alimentation" value={fmt(INCI.H_feed_water_kW, 0)}           unit="kW" />
            <KV label="H gaz recyclés"     value={fmt(INCI.H_fumee_recy_kW, 3)}           unit="kW" />
            <KV label="H combustible app." value={fmt(INCI.H_combustible_appoint_kW, 3)}  unit="kW" />
            <KV label="H air parasites"    value={fmt(INCI.H_tot_air_entrant_kW, 3)}      unit="kW" />
          </SubSection>
        </div>
      </Section>

      {/* 2. Gaz de combustion */}
      <Section title={t('2. Gaz de combustion')}>
        <div style={styles.twoCol}>
          <SubSection>
            <KV label={t('Température de sortie')}     value={fmt(T_OUT, 0)}          unit="°C" />
            <KV label={t('O₂ mesuré (sec)')}           value={fmt(O2_calcule, 2)}     unit="%" />
            <KV label={t('Débit humide total [kg/h]')} value={fmt(FG_wet_total, 0)} />
      ...
      <Section title={t('3. Émissions polluantes')}>
      ...
      {/* 4. OPEX */}
      <Section title={t('OPEX — Coûts horaires')}>
        <div style={styles.twoCol}>
          <SubSection title={t('Consommations électriques')}>
            {elecRows.length > 0
              ? <ElecTable rows={elecRows} t={t} />
              : <span style={{ color: '#999', fontSize: 12 }}>{t('Données non disponibles (ouvrir l\'onglet Design)')}</span>}
          </SubSection>
          <div>
            {waterConsumption.length > 0 && (
              <SubSection title={t('Consommation d\'eau')}>
...
const styles = {
  ...
  mainTitle:    { fontSize: 20, fontWeight: 'bold', color: '#1a3a6b', borderBottom: '3px solid #fb8c00', paddingBottom: 8, marginBottom: 24 },
  section:      { marginBottom: 28, border: '1px solid #d0daea', borderRadius: 6, overflow: 'hidden' },
  sectionTitle: { fontSize: 15, fontWeight: 'bold', color: '#fff', background: '#fb8c00', margin: 0, padding: '8px 14px' },
  ...
  th:           { background: '#fff3e0', border: '1px solid #ffe0b2', padding: '4px 6px', textAlign: 'center', fontWeight: 'bold', color: '#e65100' },
  ...
  tag:          { background: '#fff8f0', border: '1px solid #ffe0b2', borderRadius: 4, padding: '4px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 110 },
  tagLabel:     { fontSize: 10, color: '#555' },
  tagValue:     { fontSize: 13, fontWeight: 'bold', color: '#e65100' },
  footer:       { marginTop: 24, textAlign: 'right', fontSize: 11, color: '#999', borderTop: '1px solid #eee', paddingTop: 8 },
};

export default GF_Report;

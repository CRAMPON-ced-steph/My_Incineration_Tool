...
const FB_Retro_Rapport = ({ calculationResult, nodeData, inputParams, onClose }) => {
      ...
      <div style={styles.modal}>
        ...
        <div style={styles.header}>
          <h2 style={styles.headerTitle}>
            Four à lit fluidisé — Rapport rétro-calcul
          </h2>
        ...
        <div style={styles.scrollArea}>
          <div style={styles.reportContent}>
            <h1 style={styles.mainTitle}>
              Rapport de synthèse — Mode rétro-calcul FB
            </h1>
            ...
            <Section title="1. Paramètres d'entrée">
              <div style={styles.twoCol}>
                <SubSection title="Conditions de calcul">
                  <KV
                    label="Type de bilan"
                    value={isDS ? 'Calcul du MS (Q boue connu)' : 'Calcul du Q boue (MS connu)'}
                  />
                  <KV label="Type de déchet" value={p.wasteType || '—'} />
                  <KV label="Température de l'air" value={fmt(p.Tair_FB_C, 1)} unit="°C" />
                  <KV label="Pertes thermiques" value={fmt(p.Thermal_losses_MW, 2)} unit="MW" />
                </SubSection>
                <SubSection title="Données boue">
                  {isDS ? (
                    <>
                      <KV label="Q boue (imposé)" value={fmt(p.Q_boue_kg_h, 0)} unit="m³/h" />
                      <KV label="MV (imposé)" value={fmt(p.MV_percent, 1)} unit="%" />
                      <KV label="MS (calculé)" value={fmt(r.MS, 1)} unit="%" />
                    </>
                  ) : (
                    <>
                      <KV label="MS (imposé)" value={fmt(p.MS_percent, 1)} unit="%" />
                      <KV label="MV (imposé)" value={fmt(p.MV_percent, 1)} unit="%" />
                      <KV label="Q boue (calculé)" value={fmt(r.Q_boue_kg_h, 0)} unit="kg/h" />
                    </>
                  )}
                  <KV label="PCI / MV" value={fmt(r.PCIkcalkgMV, 0)} unit="kcal/kgMV" />
                </SubSection>
              </div>
            </Section>
            ...
            <Section title="3. Résultats du rétro-calcul">
              <div style={styles.twoCol}>
                ...
                <SubSection title="Déchets traités">
                  <KV label="Débit boue" value={fmt(r.MasseDechet, 0)} unit="kg/h" />
                  <KV
                    label="Débit boue"
                    value={fmt((parseFloat(r.MasseDechet) || 0) / 1000, 2)}
                    unit="t/h"
                  />
                  <KV label="MS" value={fmt(r.MS, 1)} unit="%" />
            ...
            <Section title="4. Synthèse">
              <div style={styles.tagRow}>
                {[
                  ...
                  {
                    label: 'Débit boue [kg/h]',
                    val: fmt(r.MasseDechet, 0),
                    color: '#2ecc71',
                  },
                  {
                    label: 'PCI [kcal/kg]',
                    val: fmt(r.calculatePCIkcalkg, 0),
                    color: '#f39c12',
                  },
                  {
                    label: 'PCI [kJ/kg]',

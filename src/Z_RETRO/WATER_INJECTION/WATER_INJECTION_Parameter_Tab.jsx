...
import { performCalculation_WATER_INJECTION_option_T } from './WATER_INJECTION_calculations';
import { performCalculation_WATER_INJECTION_option_Qeau } from './WATER_INJECTION_calculations2';
...
const DEFAULT_VALUES = {
  Teau: '15',
  T_amont_WATER_INJECTION: '10',
  Qeau: '0',
  PDC_aero: '10',
  bilanType: BALANCE_TYPES.TEMPERATURE
};

const WATER_INJECTION_Parameter_Tab = ({ nodeData, title, onSendData, onClose, currentLanguage }) => {
  // États principaux
  const [Teau, setTeau] = useState(() =>
    localStorage.getItem(STORAGE_KEYS.T_EAU) || DEFAULT_VALUES.Teau
  );
  const [T_amont_WATER_INJECTION, setT_amont_WATER_INJECTION] = useState(() =>
    localStorage.getItem(STORAGE_KEYS.T_AMONT_WATER_INJECTION) ||
    nodeData?.result?.dataFlow?.T?.toString() ||
    DEFAULT_VALUES.T_amont_WATER_INJECTION
  );
  ...
  const [calculationResult_WATER_INJECTION, setCalculationResult_WATER_INJECTION] = useState(() => {
    try {
  ...
  useEffect(() => {
    const saveToLocalStorage = () => {
      try {
        const dataToSave = {
          ...
          [STORAGE_KEYS.T_AMONT_WATER_INJECTION]: T_amont_WATER_INJECTION,
    ...
    saveToLocalStorage();
  }, [Teau, T_amont_WATER_INJECTION, Qeau, PDC_aero, bilanType]);

  // Sauvegarde des résultats de calcul
  useEffect(() => {
    if (calculationResult_WATER_INJECTION) {
      try {
        localStorage.setItem(STORAGE_KEYS.CALCULATION_RESULT, JSON.stringify(calculationResult_WATER_INJECTION));
      } catch (error) {
  ...
  const validateInputs = useCallback(() => {
    const inputs = {
      ...
      T_amont_WATER_INJECTION: parseFloat(T_amont_WATER_INJECTION),
    ...
    if (inputs.T_amont_WATER_INJECTION < -T_ref) {
    ...
    return inputs;
  }, [Teau, T_amont_WATER_INJECTION, Qeau, PDC_aero, t]);
  ...
  const handleSendData = useCallback(async () => {
    ...
    try {
      ...
      if (bilanType === BALANCE_TYPES.TEMPERATURE) {
        result = performCalculation_WATER_INJECTION_option_T(
          nodeData,
          validatedInputs.T_amont_WATER_INJECTION,
          validatedInputs.Teau,
          validatedInputs.PDC_aero
        );
      } else if (bilanType === BALANCE_TYPES.WATER_FLOW) {
        result = performCalculation_WATER_INJECTION_option_Qeau(
          nodeData,
  ...
  const clearMemory = useCallback(() => {
    try {
      ...
      setCalculationResult_WATER_INJECTION(null);

      // Réinitialisation aux valeurs par défaut
      setTeau(DEFAULT_VALUES.Teau);
      setT_amont_WATER_INJECTION(nodeData?.result?.dataFlow?.T?.toString() || DEFAULT_VALUES.T_amont_WATER_INJECTION);
  ...
  const handleTAmontChange = createInputHandler(setT_amont_WATER_INJECTION, DEFAULT_VALUES.T_amont_WATER_INJECTION);
    ...
    <div className="container-box">
      ...
      <div className="prez-3-buttons">
        ...
        <ShowResultButton
          ...
          currentLanguage={currentLanguage}
          disabled={!calculationResult_WATER_INJECTION}
        />
      ...
      <div style={{ marginTop: '12px' }}>
        <button
          ...
          disabled={!calculationResult_WATER_INJECTION || isCalculating}
          style={{ width: '100%', padding: '8px 16px', background: calculationResult_WATER_INJECTION ? '#1a3a6b' : '#ccc', color: '#fff', border: 'none', borderRadius: '4px', cursor: calculationResult_WATER_INJECTION ? 'pointer' : 'not-allowed', fontWeight: 'bold', fontSize: '13px' }}
        >
          Editer Rapport
        </button>
      </div>

      {showReport && calculationResult_WATER_INJECTION && (
        <WATER_INJECTION_Retro_Rapport
          calculationResult={calculationResult_WATER_INJECTION}
          inputParams={{ Teau, T_amont_WATER_INJECTION, Qeau, PDC_aero, bilanType }}
...
export default React.memo(WATER_INJECTION_Parameter_Tab);

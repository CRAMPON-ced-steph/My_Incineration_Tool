...
// performCalculation_CYCLONE_option_T
export const performCalculation_CYCLONE = (nodeData, T_air_decolmatation, Q_air_decolmatation,T_amont_CYCLONE , PDC_aero) => {
  
  
  // Extract receivedData from nodeData.result with default values
  // LECTURE DU NOEUD PRECEDENT
  const T_aval_CYCLONE = nodeData.result.dataFlow.T;
   const T =parseFloat(T_amont_CYCLONE) || 0;
 // const T_amont_CYCLONE = parseFloat(T_amont_CYCLONE);
...
const Qv_air_entrant_tot_Nm3_h = AIR_DILUTION_T(Tair, T_amont_CYCLONE, T_aval_CYCLONE,Qm_CO2_out_kg_h, Qm_H2O_out_kg_h, Qm_N2_out_kg_h, Qm_O2_out_kg_h);
...
const Qair_parasite = Qv_air_entrant_Nm3_h-Q_air_decolmatation;
  ...
  const H_H2O_kj = (fh_H2O(T_amont_CYCLONE) + 540 * 4.18) * Qm_H2O_kg_h;
  const H_O2_kj = fh_O2(T_amont_CYCLONE) * Qm_O2_kg_h;
  const H_N2_kj = fh_N2(T_amont_CYCLONE) * Qm_N2_kg_h;

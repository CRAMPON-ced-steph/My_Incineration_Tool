...
// performCalculation_BHF_option_T
export const performCalculation_BHF = (nodeData, T_air_decolmatation, Q_air_decolmatation,T_amont_BHF, PDC_aero ) => {
  
  
  // Extract receivedData from nodeData.result with default values
  // LECTURE DU NOEUD PRECEDENT
  const T_aval_BHF = nodeData.result.dataFlow.T;
   //let T =parseFloat(T_amont_BHF) || 0;
 // const T_amont_BHF = parseFloat(T_amont_BHF);
...
const Qv_air_injecté_net_Nm3_h = Qv_air_entrant_Nm3_h-Q_air_decolmatation;
  ...
  const H_CO2_kj = fh_CO2(T_amont_BHF) * Qm_CO2_kg_h;
  const H_H2O_kj = (fh_H2O(T_amont_BHF) + Lv) * Qm_H2O_kg_h;
  const H_O2_kj = fh_O2(T_amont_BHF) * Qm_O2_kg_h;
  const H_N2_kj = fh_N2(T_amont_BHF) * Qm_N2_kg_h;
...
const dataBHF={
  ...
  Qv_air_decolmatage_Nm3_h: parseFloat(Q_air_decolmatation) || 0,

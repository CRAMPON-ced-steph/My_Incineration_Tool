...
export default function BHF_Retro_Rapport({ calculationResult, inputParams, onClose }) {
  const df = calculationResult?.dataFlow || {};
  const db = calculationResult?.dataBHF || {};

  return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        <div style={styles.header}>
          <h1 style={styles.headerTitle}>Filtre à manches (BHF) — Rapport Rétro</h1>
          <button style={styles.closeBtn} onClick={onClose}>✕ Fermer</button>
        </div>
        <div style={styles.scrollArea}>
          <div style={styles.reportContent}>
            <div style={styles.mainTitle}>Filtre à manches (BHF)</div>
            ...
            <Section title="2. Fumées en entrée (côté amont BHF)">
              <SubSection>
                <GasTable df={df} />
              </SubSection>
              <SubSection>
            ...
            {/* Section 4 — Air parasite BHF */}
            <Section title="4. Air parasite (BHF)">
              <SubSection>
                <div style={styles.twoCol}>
                  <div>
                  ...
                  <div>
                    <KV label="O₂ air entrant" value={fmt(db.Qv_O2_air_entrant_Nm3_h, 0)} unit="Nm³/h" />
                    <KV label="O₂ air entrant" value={fmt(db.Qm_O2_air_entrant_kg_h, 0)} unit="kg/h" />
                    <KV label="N₂ air entrant" value={fmt(db.Qv_N2_air_entrant_Nm3_h, 0)} unit="Nm³/h" />
                    <KV label="N₂ air entrant" value={fmt(db.Qm_N2_air_entrant_kg_h, 0)} unit="kg/h" />
                    <KV label="Pression sortie" value={fmt(df.P_mmCE, 1)} unit="mmCE" />
                  </div>
                </div>
              </SubSection>
            ...
            <div style={styles.footer}>Rapport généré automatiquement — BHF Retro</div>
          </div>
        </div>
      </div>
    </div>

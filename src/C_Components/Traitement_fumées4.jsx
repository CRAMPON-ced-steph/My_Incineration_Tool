...
const FGT = ({ masses, innerData }) => {
  ...
  const calculateValues = (row) => {
    ...
    // Mise à jour des données dans innerData
    ...
    // Mise à jour de innerData pour HCl
  ...
  const hclRow = rows.find((r) => r.pollutant === 'HCl');
  if (hclRow) {
    innerData['mass_reduction_HCl'] = hclRow.mass_reduction;
  }
          ...
          {rows.map((row, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td>{calculatedRow.mass_reactif_st.toFixed(3)}</td>
